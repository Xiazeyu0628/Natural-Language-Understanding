...
def get_args():
    ...
    parser.add_argument('--checkpoint-path', default='results/deeper_network/checkpoint_best.pt', help='path to the model file')
...
def main(args):
    ...
    state_dict = torch.load(args.checkpoint_path, map_location=lambda s, l: default_restore_location(s, 'cpu'))    # 把最优模型读进来了
    ...
    model = models.build_model(args, src_dict, tgt_dict)   # 用source字典和target字典的大小来定义了这个model
    if args.cuda:
        model = model.cuda()
    model.eval()
    ...
    for i, sample in enumerate(progress_bar):
        with torch.no_grad():
           ...
           # src_token = sample['src_tokens']
           # src_token_size = src_token.size()   # batch_size * length
            go_slice = \
                torch.ones(sample['src_tokens'].shape[0],1).fill_(tgt_dict.eos_idx).type_as(sample['src_tokens'])
            # eos_index = 1   eos 是代表上个句子结束的token
            #_fill 填充

            # type_as 进行数据类型转换
            prev_words = go_slice   # size batch_size * 1   10*1
            prev_words_size = prev_words.size()
            next_words = None

        for _ in range(args.max_len):   # 25
            with torch.no_grad():
                # Compute the decoder output by repeatedly feeding it the decoded sentence prefix 前缀
                decoder_out, _ = model.decoder(prev_words, encoder_out)   # [10,11,4420]
            # Suppress <UNK>s
            next_candidates_scores, next_candidates = torch.topk(decoder_out, 2, dim=-1)  # 沿给定dim维度返回输入张量input中 k 个最大值   ]
            # next_candidates size   10*1*2
            best_candidates = next_candidates[:, :, 0]
            backoff_candidates = next_candidates[:, :, 1]
            next_words = torch.where(best_candidates == tgt_dict.unk_idx, backoff_candidates, best_candidates)   # torch.where(condition, 满足输出x, y)
