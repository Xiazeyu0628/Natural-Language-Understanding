...
def init_logging_translate(args):
    handlers = [logging.StreamHandler()]
    if hasattr(args, 'translate_log_file') and args.translate_log_file is not None:
        os.makedirs(os.path.dirname(args.translate_log_file), exist_ok=True)
        handlers.append(logging.FileHandler(args.translate_log_file, mode='w'))
    logging.basicConfig(handlers=handlers, format='[%(asctime)s] %(message)s', datefmt='%Y-%m-%d %H:%M:%S',
                        level=logging.INFO)
    logging.info('COMMAND: %s' % ' '.join(sys.argv))
    logging.info('Arguments: {}'.format(vars(args)))
...
def _get_full_incremental_state_key(module_instance, key):
    ...
    if not hasattr(module_instance, '_fairseq_instance_id'):    # hasattr() 函数用于判断对象是否包含对应的属性。
