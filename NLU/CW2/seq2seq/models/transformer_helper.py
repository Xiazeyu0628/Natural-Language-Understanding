...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.fc1 = generate_linear(self.embed_dim, args.encoder_ffn_embed_dim)  # 全连接层 fully connect
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        residual = state.clone()   # residual is a copy of state
        ...
        state += residual   # skip connect
        state = self.self_attn_layer_norm(state)  # normalization
        ...
        state = F.dropout(state, p=self.dropout, training=self.training)
        state += residual # skip connect
        state = self.final_layer_norm(state) # normalization
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
                ...
                self_attn_mask=None,    # 用来防止读到subsequent outputs
                self_attn_padding_mask=None,  # 用来防止读到padding的位置
                need_attn=False,
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.head_embed_size = embed_dim // num_attn_heads  # this is d_k in the paper  向下取整
        ...
        nn.init.xavier_uniform_(self.k_proj.weight, gain=1 / math.sqrt(2))   # w^K
        nn.init.xavier_uniform_(self.v_proj.weight, gain=1 / math.sqrt(2))   # w^v
        nn.init.xavier_uniform_(self.q_proj.weight, gain=1 / math.sqrt(2))   # w^q
...
def generate_embedding(num_embeddings, embedding_dim, padding_idx):
    ...
    m = nn.Embedding(num_embeddings, embedding_dim, padding_idx=padding_idx)   # size of Embedding layer 5047*128
    # Fills the input Tensor with values drawn from the normal distribution,初始化embedding层的weight
    nn.init.normal_(m.weight, mean=0, std=embedding_dim ** -0.5)
    nn.init.constant_(m.weight[padding_idx], 0)    # Fills the input Tensor with the value \text{val}val
    return m


def generate_linear(in_features, out_features, bias=True):
    m = nn.Linear(in_features, out_features, bias)
    nn.init.xavier_uniform_(m.weight)  # Fills the input Tensor with values
    if bias:
