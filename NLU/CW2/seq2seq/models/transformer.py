...
class TransformerEncoder(Seq2SeqEncoder):
    ...
    def __init__(self,
        ...
        self.padding_idx = dictionary.pad_idx    # padding id 0
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        for layer in self.layers:
            forward_state = layer(state=forward_state, encoder_padding_mask=encoder_padding_mask) # [batch_size, src_time_steps]
...
class TransformerDecoder(Seq2SeqDecoder):
    ...
    def buffered_future_mask(self, tensor):
        ...
        if (not hasattr(self, '_future_mask')) or self._future_mask is None or self._future_mask.size(0) < dim:
            self._future_mask = torch.triu(fill_with_neg_inf(tensor.new(dim, dim)), diagonal=1)
