...
def get_args():
    ...
    parser.add_argument('--translate-log-file', default=None, help='path to save training logs')
...
def main(args):
    ...
    # Load dictionaries
    #
    # load europarl_prepared文件夹里的 dict.en, dict.de 这种处理好的
    #src_dict =  Dictionary.load("/Users/xiazeyu/nlu_cw2/data-bin/dict.de")
    # tgt_dict = Dictionary.load("/Users/xiazeyu/nlu_cw2/data-bin/dict.en")
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        model.train()
        stats = OrderedDict()   # 有序字典，输出时和定义时的顺序时
        stats['loss'] = 0
        stats['lr'] = 0
        stats['num_tokens'] = 0
        stats['batch_size'] = 0
        ...
        for i, sample in enumerate(progress_bar):
            ...
            output, _ = model(sample['src_tokens'], sample['src_lengths'], sample['tgt_inputs'])   # decoder_output, attn_weights
        ...
        if epoch==(args.max_epoch-1):
            test(args, model, criterion, test_dataset, epoch)
        else:
...
def test(args, model, criterion, test_dataset, epoch):
    """ Validates model performance on a held-out development set. """
    valid_loader = \
        torch.utils.data.DataLoader(test_dataset, num_workers=1, collate_fn=test_dataset.collater,
                                    batch_sampler=BatchSampler(test_dataset, args.max_tokens, args.batch_size, 1, 0,
                                                               shuffle=False, seed=42))
    model.eval()
    stats = OrderedDict()
    stats['test_loss'] = 0
    ...
    # Calculate validation perplexity
    stats['test_loss']  = stats['test_loss']  / stats['num_tokens']
    perplexity = np.exp(stats['test_loss'] )
    stats['num_tokens'] = stats['num_tokens'] / stats['batch_size']

    logging.info(
        'Epoch {:03d}: {}'.format(epoch, ' | '.join(key + ' {:.3g}'.format(value) for key, value in stats.items())) +
        ' | test_perplexity {:.3g}'.format(perplexity))
